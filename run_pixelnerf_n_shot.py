...
from pixelnerf_dataset_nshot import PixelNeRFDatasetNshot
...
def set_up_test_data(train_dataset, device):
    ...
    source_pose_idx_list  = train_dataset.test_source_pose_idx_list
    source_img_f_list = [f"{obj_dir}/{str(source_pose_idx).zfill(z_len)}.npy" for source_pose_idx in source_pose_idx_list]
    source_image_list = [np.load(source_img_f) / 255 for source_img_f in source_img_f_list]
    source_pose_list = [train_dataset.poses[obj_idx, source_pose_idx] for source_pose_idx in source_pose_idx_list]
    ...
    R_list = [torch.Tensor(source_R.T @ target_R).to(device) for source_R in source_R_list]
    # R = torch.Tensor(source_R.T @ target_R).to(device)

    for i,source_image in enumerate(source_image_list):
        source_image_path = f"/data/home1/saichandra/Vardhan/projectAIP/pytorch-nerf/results/pixel/oneshot/03790512/source_image{i}.png"
        plt.imshow(source_image)
        plt.savefig(source_image_path)
        plt.close()

    source_image_list_tensor = torch.Tensor(np.array(source_image_list))    
    temp = [((source_image_list_tensor[i] - train_dataset.channel_means) / train_dataset.channel_stds) for i in range(source_image_list_tensor.shape[0])]
    final_source_image_list = [temp[i].unsqueeze(0).permute(0, 3, 1, 2) for i in range(len(temp))]    
    # print('Length is ',len(final_source_image_list))
    # print(final_source_image_list[0].shape)
    source_image_list = torch.stack(final_source_image_list)
    source_image_list_tensor = source_image_list.to(device)
    ...
    return source_image_list_tensor, R_list, target_image
