...
class VeryTinyNeRFMLP(nn.Module):
    ...
    def forward(self, xs, ds):
        ...
        outputs = self.middle_mlp(inp_middle_mlp)
        sigma_is = outputs[:, 0]
...
def main(device="cuda:0"):
    ...
    plt.savefig("results/tiny_param_6/{}/test_img.png".format(folder_name))
    ...
    for i in range(num_iters):
        ...
        C_rs_c = run_one_iter_of_tiny_nerf(ds, N_c, t_i_c_bin_edges, t_i_c_gap, os, chunk_size, F_c)
        ...
        if i % display_every == 0:
            ...
            plt.title("PSNR")
            if i % plot_every == 0:
                plt.savefig("results/tiny_param_6/{}/iter_{}.png".format(folder_name,i))
            F_c.train()
            plt.close()
    ...
    np.save("results/tiny_param_6/{}/psnrs.npy".format(folder_name), psnrs)
    logging.info("Saving model")
    torch.save(F_c.state_dict(), "results/tiny_param_6/{}/F_c.pth".format(folder_name))
    logging.info("Saving optimizer")
    torch.save(optimizer.state_dict(), "results/tiny_param_6/{}/optimizer.pth".format(folder_name))
    logging.info("DONE!")

if __name__ == "__main__":
    device = "cuda:2"
    logging.basicConfig(filename='tiny_nerf_6.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
