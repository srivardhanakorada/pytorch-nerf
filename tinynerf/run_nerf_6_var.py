...
class VeryTinyNeRFMLP(nn.Module):
    ...
    def forward(self, xs, ds):
        ...
        outputs = self.middle_mlp(outputs)
        sigma_is = outputs[:, 0]
        # c_is = self.late_mlp(torch.cat([outputs[:, 1:], ds_encoded], dim=-1))
        c_is = self.late_mlp(outputs)
...
def main(device="cuda:0",num_iters=20000):
    ...
    results_folder = f"/data/home1/saichandra/Vardhan/projectAIP/pytorch-nerf/results/tiny_nerf/tiny_param_6_var/{name}"
    seed = 9458
...
if __name__ == "__main__":
    device = "cuda:1"
    logging.basicConfig(filename='/data/home1/saichandra/Vardhan/projectAIP/pytorch-nerf/logs/tiny_nerf/tiny_param_6_var/bike.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
